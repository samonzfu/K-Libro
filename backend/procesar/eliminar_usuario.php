...
if (!$usuario_id_borrar || $usuario_id_borrar < 1) {
    header('Location: ../../frontend/7_Admin/panel_admin.php?error=id_invalido');
    exit;
}

// 5. No puede eliminarse a sí mismo
if ($usuario_id_borrar === (int) $_SESSION['user_id']) {
    header('Location: ../../frontend/7_Admin/panel_admin.php?error=autoeliminar');
    exit;
}
...
if (!$objetivo) {
    header('Location: ../../frontend/7_Admin/panel_admin.php?error=no_existe');
    exit;
}

if ($objetivo['rol'] === 'admin') {
    header('Location: ../../frontend/7_Admin/panel_admin.php?error=es_admin');
    exit;
}

// 7. Eliminar — las FKs ON DELETE CASCADE se ocupan del resto
$stmt = $pdo->prepare("DELETE FROM usuarios WHERE id = ? AND rol = 'user'");
$stmt->execute([$usuario_id_borrar]);

header('Location: ../../frontend/7_Admin/panel_admin.php?ok=usuario_eliminado');
exit;
